...
def dataset_reader(filename_queue, config):  # prev name: CamVid_reader
    ...
    image_bytes = [tf.image.decode_png(imageValue,channels=config["INPUT_CHANNELS"])]
    label_bytes = [tf.image.decode_png(labelValue,channels=1)]# Labels are png, not jpeg
...
def dataset_inputs(image_filenames, label_filenames, batch_size, config):
    ...
    #filename_queue = tf.data.Dataset.from_tensor_slices(tuple([images, labels]))
...
def _generate_image_and_label_batch(image, label, min_queue_examples,
    ...
    print("*******************\n Have fun\n *******************")
    print(image)
    images=tf.data.Dataset.from_tensors(image).repeat().batch(batch_size)
    label_batch=tf.data.Dataset.from_tensors(label).batch(batch_size)
...
def get_all_test_data(im_list, la_list):
    ...
    for im_filename, la_filename in zip(im_list, la_list):
        im = imread(im_filename,pilmode='L')
